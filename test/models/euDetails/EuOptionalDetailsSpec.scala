...
class EuOptionalDetailsSpec extends SpecBase with Matchers {
  ...
  "EuOptionalDetails" - {
    ...
    "must deserialize from JSON correctly" in {
      ...
      val expectedEuDetails = EuOptionalDetails(
        euCountry = Country("DE", "Germany"),
        hasFixedEstablishment = Some(true),
        registrationType = Some(RegistrationType.TaxId),
        euVatNumber = Some("DK12345678"),
      ...
      json.validate[EuOptionalDetails] mustBe JsSuccess(expectedEuDetails)
    }
    ...
    "must handle missing fields during deserialization" in {
      ...
      json.validate[EuOptionalDetails] mustBe a[JsError]
    }
