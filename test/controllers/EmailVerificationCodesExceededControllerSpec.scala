...
class EmailVerificationCodesExceededControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  "EmailVerificationCodesExceeded Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val notModifyingMode: RegistrationModificationMode = NotModifyingExistingRegistration

      val application = applicationBuilder(userAnswers = Some(basicUserAnswersWithVatInfo)).build()

      running(application) {
        val request = FakeRequest(GET, routes.EmailVerificationCodesExceededController.onPageLoad(waypoints).url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[EmailVerificationCodesExceededView]

        val redirectLink: String = BusinessContactDetailsPage.route(waypoints).url

        status(result) `mustBe` OK
        contentAsString(result) `mustBe` view(redirectLink, notModifyingMode)(request, messages(application)).toString
      }
    }
