...
class GoodsConsignmentValueControllerSpec extends SpecBase with MockitoSugar {

  private val waypoints: Waypoints = EmptyWaypoints

  val formProvider = new GoodsConsignmentValueFormProvider()
  val form: Form[Boolean] = formProvider()

  lazy val goodsConsignmentValueRoute: String = routes.GoodsConsignmentValueController.onPageLoad(waypoints).url

  "GoodsConsignmentValue Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, goodsConsignmentValueRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[GoodsConsignmentValueView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(GoodsConsignmentValuePage, true).success.value
    ...
    "must save and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, goodsConsignmentValueRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(GoodsConsignmentValuePage, true).success.value

        status(result) mustBe SEE_OTHER
        redirectLocation(result).value mustBe GoodsConsignmentValuePage.navigate(waypoints, emptyUserAnswers, expectedAnswers).route.url
