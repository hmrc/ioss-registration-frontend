...
import forms.filters.BusinessBasedInNiFormProvider
...
import views.html.filters.BusinessBasedInNiView
  ...
  "BusinessBasedInNi Controller" - {
    ...
    "must save and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustBe BusinessBasedInNiPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).route.url
