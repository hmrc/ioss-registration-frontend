  ...
  "NorwegianBasedBusiness Controller" - {
    ...
    "must save and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(NorwegianBasedBusinessPage, true).success.value

        status(result) mustBe SEE_OTHER
        redirectLocation(result).value mustBe NorwegianBasedBusinessPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).route.url
    ...
    "must redirect to Registered for IOSS in Eu for a GET when there are no user answers" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, norwegianBasedBusinessRoute)
