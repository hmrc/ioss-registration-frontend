...
import forms.filters.RegisteredForIossInEuFormProvider
...
class RegisteredForIossInEuControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider = new RegisteredForIossInEuFormProvider()
  val form: Form[Boolean] = formProvider()

  lazy val registeredForIossInEuRoute: String = routes.RegisteredForIossInEuController.onPageLoad(waypoints).url

  "RegisteredForIossInEu Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, registeredForIossInEuRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[RegisteredForIossInEuView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(RegisteredForIossInEuPage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, registeredForIossInEuRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(RegisteredForIossInEuPage, true).success.value

        status(result) mustBe SEE_OTHER
        redirectLocation(result).value mustBe RegisteredForIossInEuPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).route.url
