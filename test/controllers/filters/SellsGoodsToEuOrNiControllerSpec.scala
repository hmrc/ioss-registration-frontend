...
import forms.filters.SellsGoodsToEuOrNiFormProvider
import models.UserAnswers
import org.mockito.ArgumentMatchers.{any, eq => eqTo}
import org.mockito.Mockito.{times, verify, when}
import org.scalatestplus.mockito.MockitoSugar
import pages.filters.SellsGoodsToEuOrNiPage
...
class SellsGoodsToEuOrNiControllerSpec extends SpecBase with MockitoSugar {

  private val waypoints: Waypoints = EmptyWaypoints

  val formProvider = new SellsGoodsToEuOrNiFormProvider()
  val form: Form[Boolean] = formProvider()

  lazy val sellsGoodsToEuOrNiRoute: String = routes.SellsGoodsToEuOrNiController.onPageLoad(waypoints).url

  "SellsGoodsToEuOrNi Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, sellsGoodsToEuOrNiRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[SellsGoodsToEuOrNiView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(SellsGoodsToEuOrNiPage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, sellsGoodsToEuOrNiRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(SellsGoodsToEuOrNiPage, true).success.value

        status(result) mustBe SEE_OTHER
        redirectLocation(result).value mustBe SellsGoodsToEuOrNiPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).route.url
