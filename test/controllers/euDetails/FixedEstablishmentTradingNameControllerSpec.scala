...
class FixedEstablishmentTradingNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  "FixedEstablishmentTradingName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = answers.set(FixedEstablishmentTradingNamePage(countryIndex), "answer").success.value
    ...
    "must save and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = answers.set(FixedEstablishmentTradingNamePage(countryIndex), "answer").success.value

        status(result) mustBe SEE_OTHER
        redirectLocation(result).value mustBe FixedEstablishmentTradingNamePage(countryIndex).navigate(waypoints, answers, expectedAnswers).url
