...
class CheckBouncedEmailFilterImpl()(implicit val executionContext: ExecutionContext)
  extends ActionFilter[AuthenticatedMandatoryIossRequest] {
  ...
  override protected def filter[A](request: AuthenticatedMandatoryIossRequest[A]): Future[Option[Result]] = {
    ...
    if (request.registrationWrapper.registration.schemeDetails.unusableStatus && emailMatched) {
      ...
      Future(Some(Redirect(controllers.routes.BusinessContactDetailsController.onPageLoad(changeRegWaypoint))))
    } else {
      Future(None)
    }
