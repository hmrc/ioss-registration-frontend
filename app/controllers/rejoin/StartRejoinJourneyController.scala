  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = cc.authAndRequireIoss(RejoiningRegistration, waypoints = waypoints).async {
    implicit request: AuthenticatedMandatoryIossRequest[AnyContent] =>
      (for {
        ...
        vatCustomerInfoResponse <- registrationConnector.getVatCustomerInfo()
      } yield {
        ...
        val vatCustomerInfo = getResponseValue(vatCustomerInfoResponse)

        if (vatCustomerInfo.deregistrationDecisionDate.isDefined) {
