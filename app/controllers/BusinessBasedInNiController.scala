...
class BusinessBasedInNiController @Inject()(
  ...
  def onPageLoad(): Action[AnyContent] = cc.identifyAndGetOptionalData {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.getOrElse(UserAnswers(request.userId)).get(BusinessBasedInNiPage) match {
        case None => form
  ...
  def onSubmit(): Action[AnyContent] = cc.identifyAndGetOptionalData.async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.getOrElse(UserAnswers(request.userId)).set(BusinessBasedInNiPage, value))
            _ <- cc.sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(BusinessBasedInNiPage, updatedAnswers))
      )
  }
}
