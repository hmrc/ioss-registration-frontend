...
object PreviousIossNumberSummary  {
  ...
  def row(answers: UserAnswers, waypoints: Waypoints, countryIndex: Index, schemeIndex: Index)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PreviousIossNumberPage(countryIndex, schemeIndex)).map {
      answer =>
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", controllers.previousRegistrations.routes.PreviousIossNumberController.onPageLoad(waypoints, countryIndex, schemeIndex).url)
